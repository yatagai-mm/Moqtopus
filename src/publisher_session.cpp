...
public:
  ...
  void cancel(uint64_t reset_error_code) {
    ...
    owner_.send_plane_->detach_subscription(request_id_, reset_error_code);
...
private:
  ...
  void handle_request_update(const ControlMessage &message) {
    ...
    if (!owner_.consume_peer_request_id(update->request_id, error)) {
...
Publisher::~Publisher() { close_and_wait(); }

std::unique_ptr<Publisher> Publisher::connect(MsQuicClientConfig msquic_config, PublisherConfig publisher_config) {
  auto session = std::unique_ptr<Publisher>(new Publisher(std::move(msquic_config), publisher_config));
  session->start();
  return session;
}

Publisher::Publisher(MsQuicClientConfig msquic_config, PublisherConfig publisher_config)
    : Session(std::move(msquic_config)), publisher_config_(publisher_config),
      send_plane_(std::make_unique<SendDataPlane>(SendDataPlane::Callbacks{
...
void Publisher::register_track(PublishedTrack track) {
...
void Publisher::publish(PublishedObject object) {
  const std::lock_guard<std::recursive_mutex> lock(mutex_);
  if (state_.phase != SessionPhase::Ready) {
...
void Publisher::end_track(const TrackNamespace &track_namespace, const TrackName &track_name, PublishDoneCode code,
                          std::string reason) {
...
void Publisher::handle_data_stream(uint64_t, const std::shared_ptr<StreamContext> &stream, ByteBuffer, bool) {
...
bool Publisher::consume_peer_request_id(RequestId request_id, std::string &error) {
...
void Publisher::announce_namespace(const TrackNamespace &track_namespace) {
...
void Publisher::withdraw_namespace(const TrackNamespace &track_namespace) {
...
std::shared_ptr<StreamContext> Publisher::open_data_stream() {
  try {
...
void Publisher::terminate_subscriptions(const std::string &) {
...
size_t Publisher::active_subscriptions() const { return subscriptions_.size(); }
